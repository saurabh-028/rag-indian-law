"""
Cleaner for: criminal_law_actionable_reference.json
Sector: criminal_law
...
SECTOR   = "criminal_law"
SOURCE   = "Criminal_Law_Actionable_Reference"
...
def parse_from_json(json_path: str) -> List[Dict]:
    ...
    for mechanism in data.get("grievance_mechanisms", []):
        ...
        if rti_qs:
            rti_parts = ["## RTI sample questions for criminal law disputes", ""]
            for q in rti_qs:
                rti_parts.append(f"  - {q}")
            records.append({
                "doc_id"         : f"{SOURCE}_{mech_id}_rti",
                "section_number" : f"{mech_id}_RTI",
                "section_title"  : "RTI sample questions for criminal law disputes",
...
if __name__ == "__main__":
    import sys
    path = sys.argv[1] if len(sys.argv) > 1 else "data/criminal_law/actionable/criminal_law_actionable_reference.json"
    chunks = parse_from_json(path)
    print(f"\nParsed {len(chunks)} chunks from criminal law actionable reference")
    for c in chunks:
